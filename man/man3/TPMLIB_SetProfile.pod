...
The B<TPMLIB_SetProfile()> is used to apply one of libtpms's provided profiles
...
Once a profile has been set it will be used by the TPM 2 from then on and carried
along with the state of the TPM 2. The state of the TPM 2 must be written
using the callback registered with B<TPMLIB_RegisterCallbacks()>.
Later attempts to set a different profile will be silently ignored.
...
The state of such a TPM 2 will always be readable by the current version of
libtpms and later versions.

A profile must be a JSON map in string format. It must contain the 'name'
field with the name of a known profile to apply. It may contain a field
'algorithms' that holds a comma-separated list of algorithms to enable.

Example for valid profiles look like this:
...
The 2nd profile disables the 'sha1' and 'tdes' algorithms.
